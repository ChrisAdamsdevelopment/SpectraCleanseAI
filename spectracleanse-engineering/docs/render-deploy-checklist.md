...
**Critical**: The frontend reads `VITE_API_URL` (confirmed in `app.tsx` line 11). `.env.example` incorrectly lists `VITE_API_URL` — do not use that name. If `VITE_API_URL` is missing at build time, the frontend will throw "Missing VITE_API_URL in production build" on first load.
