...
⚠️ **Known discrepancy**: `.env.example` lists `VITE_API_URL` but `app.tsx` line 11 reads `VITE_API_URL`. The correct variable name is `VITE_API_URL`. The `.env.example` entry is incorrect and should be updated.
