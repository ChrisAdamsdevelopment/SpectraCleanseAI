...
**Frontend (build-time)**:
`VITE_API_URL` ← confirmed from `app.tsx` line 11. Note: `.env.example` lists `VITE_BACKEND_URL` — this is an **inaccuracy in .env.example**. The correct variable is `VITE_API_URL`.
...
|---|---|
| `.env.example` | Lists `VITE_BACKEND_URL`; `app.tsx` reads `VITE_API_URL` |
