...
import { calculateSeamMotionContinuity } from './MotionMetrics';
...
export function scoreLoop(anchorFrame: ExtractedFrame, endFrame: ExtractedFrame, anchor: LoopAnchor, minDurationSec = 3, maxDurationSec = 8): LoopScore {
  ...
  const motionContinuity = calculateSeamMotionContinuity(am, em) ?? 0.5;
  const motionDelta = am.motionMagnitude !== undefined && em.motionMagnitude !== undefined ? Math.abs(em.motionMagnitude - am.motionMagnitude) : undefined;
  ...
  const reasons = [
    `duration ${loopDuration.toFixed(2)}s`,
    `visual ${(visualSimilarity * 100).toFixed(0)}%`,
    `motion continuity ${(motionContinuity * 100).toFixed(0)}%${motionDelta === undefined ? '' : ` (delta ${motionDelta.toFixed(3)})`}`,
    `jump risk ${(jumpRisk * 100).toFixed(0)}%`,
  ];
