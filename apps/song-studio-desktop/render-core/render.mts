...
// (recipes + templates + buildFfmpegArgs). Uses ffmpeg-static so a real MP4 can
// be produced without the Rust/Tauri toolchain. Run with `tsx`.
...
export const nodeRenderEngine: RenderEngine = {
  async render(job: RenderJob, onLog?: RenderLogFn): Promise<RenderResult> {
    ...
    const args = buildFfmpegArgs(job, recipe, template, { fontPath: resolveFont() });
