...
// interface as the Tauri engine and reuses the SAME shared logic (recipes ->
// composition -> buildFfmpegArgs). Uses ffmpeg-static so a real MP4 can be
// produced without the Rust/Tauri toolchain. Run with `tsx`.
...
import { getRecipe } from '../src/render/recipes';
import { getTemplate } from '../src/render/templates';
import { recipeToComposition } from '../src/render/composition';
...
export const nodeRenderEngine: RenderEngine = {
  async render(job: RenderJob, onLog?: RenderLogFn): Promise<RenderResult> {
    ...
    if (recipe.audioRequired && job.audioPath && !existsSync(job.audioPath)) {
